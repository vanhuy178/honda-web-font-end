const checkValidation = (e) => {
    ...
    let valid = true;
    let checkEmaiEmpty = checkEmpty('.input-login-username', '.message-email', 'Email')
    let checkPassEmpty = checkEmpty('.input-login-password', '.message-password', 'Password');
    // check rỗng ok tiếp theo là check mail hợp lệ
    valid = checkEmaiEmpty & checkPassEmpty

    // check mail có hợp lệ hay ko
    let checkValidMail = checkEmail('.input-login-username', '.message-check-email')
    let checkValidPassword = checkPassword('.input-login-password', '.message-check-pass');
...
document.querySelector('.login__signInButton').onclick = checkValidation;
